...
if (isset($_POST['btnSubmit'])) {
    ...
    $hVaginal = $hMood = $hSagging = $hSleep = $hMemory = $hNight = $hFlashes = $hIntercourse = $hBladder = $hSugar = $hMigraine = $hBlood = $hPuffiness = $hAnxiety = $hInsomnia = $hInfertility = $hMiscarriages = $hPMS = $hBreast = $hEndometriosis = $hOsteoporosis = $hWater = $hFacial = $hAcne = $hOvaries = $hBrown = $hExercise =NULL; 
    ...
    if ($r) {
        ...
        White_Thrush, Athelete_Foot, FingerNail, Sensitivity, Weight_Gain, Think_Weight) VALUES ('$MSA_Id','$g1Tired','$g1Life','$g1Abdominal','$g1Sugar','$g1Bread','$g1Constipation','$g1Irritability','$g1Brain',
        ...
        '$g1Weight','$g1Think'
                ...
                // if ($GeneralResult)
                //     echo "GENENRAL " . $GeneralResult;
                // else
                //     echo "General Error " . mysqli_error($conn);
                ...
                $Generalquery2 = "INSERT INTO f_general_assessment_2(MSA_Id, Antibiotics, BirthControl, Steriod_Drugs, Synthetic_Hormones) VALUES ('$MSA_Id','$g2Antibiotic','$g2Birth','$g2Steroid','$g2Synthetic')";
        ...
        '$ttHealthy','$ttClean')";
                ...
                $Toxicquery3 = "INSERT INTO f_toxic_assessment_3(MSA_Id, Overeat, Chew_Food, Lower_Bowel, Hard_Sweat, Sit_Sauna, Cell_Phone, Environment, Use_Pesticides, Travel_By_Plane, Use_Computer, Live_Smoke, Use_Cleaners, 
        ...
        '$FastFood','$IceCream','$Kombucha','$Tea','$Drinks','$Restaurant')";
                ...
                if (($GeneralResult) and ($GeneralResult2) and ($ThyroidResult) and ($StressResult) and ($HormoneResult) and ($ToxicResult1) and ($ToxicResult2) and ($ToxicResult3) and ($ConsumptionResult)) {
                    ...
                    echo "<script>document.location.href='FinalResult.php?id='.$MSA_Id.'&pid='.$PatientId;'</script>";
                }
            }
        } else {
            //echo "<script>alert('Already Addedd');</script>";
            header("location:Assessment.php?id=".$PatientId);
